...
typedef struct _LUdsMsg {
    ...
    /* Result status of the network communication, see LUDS_RESULT_N_XXX */
    BYTE n_result;
...
#ifdef USE_UDSLIN_EXPORTS
#ifdef UDSLIN_EXPORTS
#define UDSLIN_API __declspec(dllexport)
#else
#define UDSLIN_API __declspec(dllimport)
#endif
#else
#define UDSLIN_API
#endif

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_SetMsgHook(PF_LUDS_MSG_HOOK hook);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_Init(void);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_Uninit(void);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_WaitForDiagRequestSent(
    LUdsMsg *req,
    LUdsMsg *rsp,
    DWORD timeout);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_WaitForDiagResponse(
    LUdsMsg *req,
    LUdsMsg *rsp,
    DWORD timeout);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_WaitForService(
    LUdsMsg *req,
    LUdsMsg *rsp);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_DiagnosticSessionControl(
    LUdsMsg *req,
    BYTE session_type);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_EcuReset(
    LUdsMsg *req,
    BYTE reset_type);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_SecurityAccess(
    LUdsMsg *req,
    BYTE level,
    BYTE *buffer,
    WORD buffer_len);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_CommunicationControl(
    LUdsMsg *req,
    BYTE ctrl_type,
    BYTE comm_type);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_TestPresent(
    LUdsMsg *req);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_ReadDataByIdentifier(
    LUdsMsg *req,
    WORD did);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_WriteDataByIdentifier(
    LUdsMsg *req,
    WORD did,
    BYTE *buffer,
    WORD buffer_len);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_ClearDiagnosticInformation(
    LUdsMsg *req,
    DWORD group);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_ReadDTCInformation(
    LUdsMsg *req,
    BYTE report_type,
    BYTE *buffer,
    WORD buffer_len);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_InputOutputControlByIdentifier(
    LUdsMsg *req,
    WORD did,
...
EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_RoutineControl(
    LUdsMsg *req,
    BYTE ctrl_type,
    WORD id,
    BYTE *buffer,
...
EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_RequestDownload(
    LUdsMsg *req,
    BYTE *mem_address,
...
EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_TransferData(
    LUdsMsg *req,
    BYTE block_seq_counter,
    BYTE *buffer,
    WORD buffer_len);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_RequestTransferExit(
    LUdsMsg *req,
    BYTE *buffer,
    WORD buffer_len);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_SetParam(
    LUdsParam param,
    void *data,
    DWORD size);

EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_GetParam(
    LUdsParam param,
    void *data,
    DWORD size);
...
EXTERNC UDSLIN_API LUdsResult __stdcall LUDS_SetLinDriver(LinDrvOps *ops);
